...
interface AuditLogInput {
  entityType: EsfAuditEntityType | string;
  entityId: string;
  action: EsfAuditAction | string;
  userId?: string;
  userEmail?: string;
...
@Injectable()
export class EsfAuditLogger {
  constructor(private readonly auditLogRepository: EsfAuditLogRepository) {}

  async log(input: AuditLogInput): Promise<EsfAuditLogEntity> {
    const auditLog = new EsfAuditLogEntity();
    auditLog.entity_type = input.entityType as EsfAuditEntityType;
    auditLog.entity_id = input.entityId;
    auditLog.action = input.action as EsfAuditAction;
    if (input.userId) {
    ...
    if (input.oldValues) {
      auditLog.old_values = this.sanitizeValues(input.oldValues);
    }
    if (input.newValues) {
      auditLog.new_values = this.sanitizeValues(input.newValues);
    }
    if (input.reason) {
